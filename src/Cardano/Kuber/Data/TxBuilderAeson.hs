...
import Cardano.Kuber.Data.Parsers (anyScriptParser, parseAddress, parseAddressBech32, parseAddressBinary, parseAddressCbor, parseAnyScript, parseAssetId, parseAssetNQuantity, parseAssetName, parseCbor, parseCborHex, parseHexString, parseScriptData, parseSignKey, parseTxIn, parseUtxo, parseUtxoCbor, parseValueText, parseValueToAsset, scriptDataParser, txInParser, txinOrUtxoParser)
import Cardano.Kuber.Error
import Cardano.Kuber.Util
import Cardano.Kuber.Utility.DataTransformation (addressInEraToAddressAny, pkhToPaymentKeyHash)
...
import Plutus.V2.Ledger.Api (CurrencySymbol, PubKeyHash (PubKeyHash, getPubKeyHash), TxOut, Validator (Validator), fromBuiltin, toBuiltin, unValidatorScript)
...
collectResolvedInputs v = case v of
  ...
  TxInputScriptUtxo tvs sd sd' m_eu umap ->
    map
      ( \v ->
          A.Object $
            A.fromList $
  ...
  TxInputReferenceScriptUtxo refSc sd sd' m_eu umap ->
    map
      ( \v ->
          A.Object $
            A.fromList $
    ...
    let txOutDatum =
          if shouldEmbed
            then case datumHashE of
              ...
              ( case datumHashE of
                  Just (Left datum) -> TxOutDatumInTx ScriptDataInBabbageEra datum
                  Just (Right dh) -> TxOutDatumHash ScriptDataInBabbageEra dh
                  _ -> TxOutDatumNone
              )
    mScript_ <- v .:? "script"
