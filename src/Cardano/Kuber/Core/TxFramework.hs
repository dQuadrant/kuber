...
executeRawTxBuilder builder pParam = do
  ...
  where
    ...
    mergeColaterals =
      foldl
        ( \(s, m) collateral -> case collateral of
            TxCollateralTxin ti -> (Set.insert ti s, m)
            TxCollateralUtxo (UTxO uto) -> (s, uto <> m)
        )
        (mempty, mempty)
        (txCollaterals builder)

    refUtxos =
      foldl
        ( \s ref -> case ref of
            TxInputReferenceTxin ti -> s
            TxInputReferenceUtxo (UTxO uto) -> uto <> s
        )
        mempty
        $ txInputReferences builder
    ...
    updatePrevGovActionsNDeposit eon enacedGovActions pParams constitutionScript (ProposalProcedureModal (ProposalProcedure (Coin co) ra ga an)) =
      let prevParamUpdate = prRoot $ Ledger.grPParamUpdate enacedGovActions
    ...
    updateConwayCert pParam ctc = case ctc of
      ConwayTxCertDeleg cdc ->
        ( case cdc of
            ...
            ConwayRegDelegCert cre del co -> pure $ ConwayRegDelegCert cre del (pParam ^. ppKeyDepositL)
        )
          <&> ConwayTxCertDeleg
      ...
      ConwayTxCertGov cgc ->
        ( case cgc of
            ...
            ConwayResignCommitteeColdKey cre _anchor -> pure $ ConwayResignCommitteeColdKey cre _anchor
        )
          <&> ConwayTxCertGov
...
-- txBuilderToTxBodyWithoutScript ::
--   (IsTxBuilderEra targetEra) =>
--   NetworkId ->
--   LedgerProtocolParameters targetEra ->
--   UTxO targetEra ->
--   TxBuilder_ targetEra ->
...
--             Map.foldlWithKey
--               ( \finalMap tag tokenNameMap ->
--                   Map.insert
--                     tag
--                     ( case Map.lookup tag finalMap of
...
--                         _ -> toJSON (HMap.singleton policyHex tokenNameMap)
--                     )
--                     finalMap
--               )
--               _metaMap
...
--           insert m1 m2' =
--             foldl
--               ( \m2 (k, v) -> case Map.lookup k m2 of
--                   Nothing -> m2
--                   Just any -> Map.insert k (any <> v) m2
--               )
--               m2'
--               $ Map.toList m1
--       meta <-
--         if null metadata -- TODO support metadata merging
...
--       collaterals <-
--         if hasScriptInput || hasPlutusMint || hasScritProposals
--           then
--             ( case maybeCollaterals of
...
--       validatedAuxScripts <-
--         mapM
--           ( \(i, sc) -> case toScriptInEra txShelleyBasedEra (txScriptToScriptAny sc) of
...
--           compulsarySignatories =
--             foldl
--               ( \acc (TxOut a _ _ _) -> case addressInEraToPaymentKeyHash a of
--                   Nothing -> acc
--                   Just pkh -> Set.insert pkh acc
--               )
--               withVoteSigs
--               $ Map.elems builderInputUtxo
--       iteration1@(txBody1, signatories, fee1) <- calculatorWithDefaults startingFee
--       let iterationFunc lastBody lastFee =calculatorWithDefaults lastFee <&> makeNewFee
...
--       applyExUnitToPartial mp onMissing pi =
--         mapM
--           ( \(k, v) -> case Map.lookup k mp of
--               Nothing -> do
...
--       applyMintExUnits mp onMissing unresolvedMints =
--         mapM
--           ( \(TxMintData (p, f) _ _) -> case Map.lookup p mp of
...
--       selectableAddrs =
--         foldl
--           ( \s selection -> case selection of
--               TxSelectableAddresses aies -> Set.fromList aies <> s
--               TxSelectableUtxos uto -> s
--               TxSelectableTxIn tis -> s
--               TxSelectableSkey sks -> Set.fromList (map (\x -> toShelleyAddr $ skeyToAddrInEra @ConwayEra x network) sks) <> s
--           )
--           Set.empty
--           selections
...
--               TxSelectableTxIn tin ->
--                 foldl
--                   ( \s tin -> case Map.lookup tin availableUtxo of
--                       Nothing -> s
--                       Just any -> Map.insert tin any s
--                   )
--                   Map.empty
--                   tin
--               TxSelectableSkey sks -> s
--           )
--           Map.empty
--           selections
...
--       selectableSkeys =
--         Map.fromList $
--           map (\x -> (skeyToPaymentKeyHash x, x)) $
--             concat
--               ( mapMaybe
--                   ( \case
...
--         any
--           ( \case
--               TxMintData tmss x0 map -> case tmss of
...
--       appendVotingSignatures old =
--         foldl
--           ( \set (TxVote (TxVoteL gai vp vo)) ->
--               let mCre = case vo of
...
--       appendCertSignatures =
--         foldl
--           ( \set item -> case getPaymentKey item of
--               Nothing -> set
...
--          in case source of
--               TxMintingSimpleScript txss ->
--                 let policy = txScriptPolicyId $ TxScriptSimple txss
--                  in do
...
--       appendMintingScriptSignatures =
--         foldl
--           ( \_set mints -> case mints of
...
--               TxOutValueShelleyBased sbe va ->
--                 let _list = valueToList (fromLedgerValue sbe va)
...
--           accum
--             ++ ( case Map.lookup txin availableUtxo of
...
--                )
--         TxCollateralUtxo (UTxO mp) ->
--           accum
--             ++ map
--               ( \(tin, TxOut a v dh _) -> case addressInEraToPaymentKeyHash a of
--                   Just pkh -> (tin, pkh)
--                   Nothing -> error "invalid address type utxo in collateral"
--               )
--               (Map.toList mp)
...
--         Nothing ->
--           if null usableAddresses
--             then
--               if null _inputs && null selections
...
--         foldr
--           ( \v addrs ->
--               -- concat $ mapMaybe findInput selections
--               case v of
...
--                 TxSelectableTxIn tis ->
--                   foldl
--                     ( \addrs2 x -> case Map.lookup x availableUtxo of
--                         Nothing -> addrs2
--                         Just (TxOut aie tov tod _) -> aie : addrs2
--                     )
--                     addrs
--                     tis
--                 TxSelectableSkey sk -> foldl (\addrs1 sk -> addrs1 ++ [skeyToAddrInEra sk network]) addrs sk
--           )
--           []
--           selections
...
--         | addChange = doAdd t
--         | otherwise =
--             let Coin minLovelace = txoutMinLovelace ledgerPParam (toCtxUTxOTxOut txout)
...
--         where
--           adaAmountToLedger era adaAmount = case valueToLovelace $ valueFromList [(AdaAssetId, Quantity adaAmount)] of
...
--                 l2 = length (valueToList (fromLedgerValue sbe2 v2))
--              in if l1 == l2
...
--       keyWitnesses =
--         if null extraSignatures
--           then TxExtraKeyWitnessesNone
--           else
--             TxExtraKeyWitnesses txAlonzoEraOnwards $
--               foldl
--                 ( \list x -> case x of
...
--                       Nothing -> list
--                       Just ha -> ha : list
--                     TxSignaturePkh (PubKeyHash pkh) -> case deserialiseFromRawBytes (AsHash AsPaymentKey) $ fromBuiltin pkh of
--                       Left _ -> list
--                       Right ha -> ha : list
--                 )
--                 []
...
--       referenceInputsFromMint =
--         foldl
--           ( \s (TxMintData tmss _ _) -> case tmss of
--               TxMintingPlutusScript tps m_eu sd -> s
--               TxMintingReferenceScript ti m_eu m_sd -> Set.insert ti s
--               TxMintingSimpleScript tss -> s
--           )
--           Set.empty
--           mintData

--       parseOutputs networkId output = case output of
--         TxOutput toc b b' _ -> case toc of
...
--         let withExUnits witF mData r mExUnit (_in, _out) = case mExUnit of
...
--       defaultExunits = ExecutionUnits {executionMemory = 0, executionSteps = 0}
...
makeTxProposals ::
  ...
  Either
    ...
    ( Bool,
      Value,
      Map (L.ProposalProcedure (ShelleyLedgerEra era)) ExecutionUnits ->
...
makeTxProposals conOnward (UTxO utxos) proposals = do
      ...
      let totalDeposit = totalProposalDeposits proposals
          unProcedures =
            mapMaybe
              ...
              proposals
          makeWitness ::
            TxProposal ConwayEra ->
            Maybe
              ( Either
                  FrameworkError
                  (ProposalProcedure (ShelleyLedgerEra ConwayEra), Either (ExecutionUnits -> ScriptWitness WitCtxStake ConwayEra) (ScriptWitness WitCtxStake ConwayEra))
              )
          makeWitness x = case x of
            ...
            TxProposalScriptReference (ProposalProcedureModal pp) eu ti ->
              Just
                ( do
       ...
       in inEonForEra
            (Left $ FrameworkError FeatureNotSupported "Proposals are not supported in Babbage era")
            ( \conwayOnward -> do
  ...
  where
    ...
    totalProposalDeposits proposals =
      lovelaceToValue $
        Coin $
...
computeBody ::
  ...
  Either
    FrameworkError
    (TxBody era, Set (Hash PaymentKey), Coin)
computeBody beraOnward cpParam@(LedgerProtocolParameters lpparam) utxo bodyContentf changeAddr signatories fixedInputSum availableInputs fixedOutputs fee = do
  ...
  let maxChange = utxoListSum availableInputs <> startingChange
      missing = filterNegativeQuantity maxChange

      (feeUsed, changeUsed, outputs) = updateOutputs mEraOnward fee change fixedOutputs
      bodyContent allOutputs =
        -- Debug.trace (" Outs: " ++ BS8.unpack (prettyPrintJSON allOutputs))
  ...
  where
    ...
    minimizeUtxos utxos remainingChange = case utxos of
      [] -> ([], remainingChange)
      (txIn, txOut@(TxOut _ txOutVal _ _)) : subUtxos ->
        if val `valueLte` remainingChange
    ...
    minimizeConsideringChange meraOnward txout f available change = case filterNegativeQuantity change of
      ...
      where
        ...
        txoutWithChange c = case txout of
          TxOut addr v md _ -> case v of
    ...
    updateOutput meraOnward _fUsed _cUsed (fee) change (txOutput : outs) =
      let (feeUsed, changeUsed, result) = transformOut _fUsed _cUsed txOutput
