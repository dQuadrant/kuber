...
async function runCip30CommitExample() {
  ...
  const hydra = new KuberHydraApiProvider("http://172.31.6.0:8082"); // Replace with your Hydra node URL

  // Load node address and test wallet signing key
  const node_addr_path = "../../kuber/kuber-hydra/devnet/credentials/alice-funds.addr";
  ...
  const testWalletSigningKey = await CardanoKeyAsync.fromCardanoCliJson(
    JSON.parse(readFileSync("../../kuber/kuber-hydra/devnet/credentials/alice-funds.sk")),
  );
